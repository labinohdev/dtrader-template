...
// [AI] invalid_digit logic disabled — with dual buy buttons (Over + Under shown together),
// no single digit is universally invalid. The wrong-direction button will fail server-side
// validation if the user picks an incompatible digit + direction (e.g., Over 9), and the
// proposal error will surface a snackbar telling them why. All 10 digits are clickable.
const LastDigitPrediction = observer(({ is_minimized }: TTradeParametersProps) => {
    const store = useTraderStore();
    const { digit_stats = [], is_market_closed, last_digit, onChange } = store;
                    ...
                    <ActionSheet.Portal shouldCloseOnDrag>
                        ...
                        <ActionSheet.Content>
                            <LastDigitSelector
                                ...
                                onDigitSelect={digit => setSelectedDigit(digit)}
